function x_next = NLSysDyn(state, input, disturbance, Dt)
% x is the current
...
v_dot = -g*(sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi))-T*cos(mu1)*sin(mu2)/m +cos(psi)*Fw/m -r*u + p*w;
w_dot = -g*(cos(phi)*sin(theta)*cos(psi)+sin(phi*sin(psi)))-T*sin(mu1)/m + sin(theta)*sin(psi)*Fw / m - p*v + q*u;
p_dot = Jinv*tau;
q_dot = Jinv * (-T*sin(mu1)*l - x_d *sin(theta) * sin(psi) *Fw);
r_dot = Jinv * (T*cos(mu1)*sin(mu2)*l + x_d*cos(psi)*Fw);
